...
function encodeConsonantWord(word) {
  ...
  let index = letters.findIndex(letter => /[aeiou]/i.test(letter));
  return word.slice(index) + '-' + word.slice(0,index) + 'ay'; // replace this!
}
/*  
...
function encodeWord(word) {
  return /^[a|e|i|o|u]/i.test(word) ? encodeVowelWord(word) : encodeConsonantWord(word);
}
...
document.getElementById("buttonEnglishToPig").addEventListener("click", () => printPigLatinText());
const outputDOM = document.createElement("p")
document.body.appendChild(outputDOM);

function printPigLatinText () {
  let input = document.getElementById("englishInput").value;
  let outputText = encodeText(input);
  outputDOM.textContent = "";
  outputDOM.textContent = outputText;
}
...
function decodeConsonantWord(word) {
  ...
  return wordArray[1].slice(0,-2) + wordArray[0];
}

console.log(decodeConsonantWord("atin-lay"));
...
function decodeWord(word) {
  return (word.includes("-")) ? decodeConsonantWord(word) : decodeVowelWord(word);
}
...
*/
document.getElementById("buttonPigToEnglish").addEventListener("click", () => printEnglishText());

function printEnglishText () {
  let input = document.getElementById("pigInput").value;
  let outputText = decodeText(input);
  outputDOM.textContent = "";
  outputDOM.textContent = outputText;
}
